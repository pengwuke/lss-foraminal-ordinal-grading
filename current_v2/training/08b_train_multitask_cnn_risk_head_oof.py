...
# -*- coding: utf-8 -*-
"""
08b_train_multitask_cnn_risk_head_oof.py
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=True,
        help=(
            "Disable during development CV so the locked test set is not "
    ...
    for images, grades, risks, indices in loader:
        ...
        if fc_l2 > 0:
            ...
            if risk_loss_weight > 0:
                penalty = penalty + model.risk_head.weight.square().sum()
            regularization = fc_l2 * penalty
        joint_loss = (
            grade_loss
        ...
        for images, grades, risks, _ in train_loader:
            ...
            with torch.autocast(
                ...
                if args.fc_l2 > 0:
                    ...
                    regularization = args.fc_l2 * penalty
                joint_loss = (
                    grade_loss
